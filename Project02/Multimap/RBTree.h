...
protected:
    ...
    int count(RBTreeNode<T> *,int)const;
    //________________________________________
...
public:
	...
	RBTreeNode<T>* findNode(const int&)const;
...
template<class T>
int RBTree<T>::count(int val)const{
    RBTreeNode<T> *tmp = findNode(val);
    if(tmp==NIL)
        return 0;
    return count(tmp,val);
}
template<class T>
int RBTree<T>::count(RBTreeNode<T> *tmp, int val)const{
    int cnt = 1;
    if(tmp->left->value.first==val)
        cnt+=count(tmp->left,val);
    if(tmp->right->value.first==val)
        cnt+=count(tmp->right,val);
    return cnt;
}

template<class T>
vector<T> RBTree<T>::retValue(int A){
    vector<T> vect;
    RBTreeNode<T> *tmp = findNode(A);
    vect.push_back(tmp->value.second);
    if(tmp!=NIL){
        if(tmp->left->value.first==A)
            vect.push_back(tmp->left->value.second);
        if(tmp->right->value.first==A)
            vect.push_back(tmp->right->value.second);
    }
    return vect;
}
template<class T>
 ...
 */
template<class T>
RBTreeNode<T>* RBTree<T>::findNode(const int &item)const{
