...
protected:
    ...
    void toVectorREC(RBTreeNode<T> *,vector<T>&)const;
    void toArrayREC(RBTreeNode<T> *,T*&,int&)const;
...
public:
    ...
    bool isEmpty()const;
    int getSize()const;
    vector<T> toVector(vector<T>) const;
    T* toArray(T*&) const;
    int count(T);
    //_________________________________________
...
int RBTree<T>::count(RBTreeNode<T> *tmp, int val)const{
    int cnt = 1;
    if(tmp->left->value==val)
        cnt+=count(tmp->left,val);
    if(tmp->right->value==val)
        cnt+=count(tmp->right,val);
    return cnt;
}

template<class T>
T* RBTree<T>::toArray(T*& A) const{
    int count = 0;
    toArrayREC(root,A,count);
    return A;
}
template<class T>
void RBTree<T>::toArrayREC(RBTreeNode<T> *tmp, T*& A, int &count) const{
    if (tmp != NIL) {
...
template<class T>
vector<T> RBTree<T>::toVector(vector<T> vect) const{
    toVectorREC(root,vect);
    return vect;
}
template<class T>
void RBTree<T>::toVectorREC(RBTreeNode<T> *tmp, vector<T>& vect) const{
    if (tmp != NIL) {
