...
int main() {
  ...
  WGraph<int,int> g1(0);
  WGraph<int,int> g2(0);
  //edge count, and ints for random vert and weight values 
  int maxVert = 100;
  int edges = 200;
  int vertFrom;
  int vertTo;
  int weight;
  //outfile for easy data collection
  ofstream out;
  out.open("out.txt");
  out<<"K Time\t\tP Time"<<endl;
  //ask user for number of edge inserts
  cout<<"\nEnter Maximum number of Vertices: ";
  //cin>>maxVert;
  cout<<"\nEnter Number of Edges: ";
  //cin>>edges;
  int originalVal = edges;
  int originalVal2 = maxVert;
  for(int iterations = 1; iterations <=10; iterations++){
      g1.clear();
      edges = originalVal*iterations;
      maxVert = originalVal2*iterations;
      cout<<endl<<endl<<"Edges: "<<edges<<" - Max Vertices: "<<maxVert;
    //creation and insertion of random vert and weight edges
    for(int i = 0; i<edges;i++){
        weight   = (rand() % 100) + 1;
        vertFrom = (rand() % maxVert) + 1;
        vertTo   = (rand() % maxVert) + 1;
        //esnsure different vert values
        while(vertTo==vertFrom)
            vertTo = (rand() % maxVert) + 1;
        g1.addEdge(vertFrom,vertTo,weight);
    }
    //minimize lines in for() loop, copy over graphs to ensure same edges.
    g2 = g1;
    //test if graph is connected, end program if not
    if(!connected(g1)){
        cout<<"\n\nThe Graph is not connected. Please run the program again."<<endl;
        out.close();
        return 0;
    }
    //test kruskal time
    start = high_resolution_clock::now();
    WGraph<int,int> krus = KruskalAlgorithm(g1);
    stop = high_resolution_clock::now();
    duration = duration_cast<microseconds>(stop - start);
    double kTime = duration.count()/1000000.0;
    //test prim time
    start = high_resolution_clock::now();
    WGraph<int,int> prim = JPAlgorithm(g2);
    stop = high_resolution_clock::now();
    duration = duration_cast<microseconds>(stop - start);
    double pTime = duration.count()/1000000.0;
    //organize and save files
    krus.sortVertexList();
    prim.sortVertexList();
    //g1.saveGraphFileGML("original");
    //krus.saveGraphFileGML("kruskal");
    //prim.saveGraphFileGML("prim");
    
    cout<<"\nKruskal Time: "<<kTime;
    cout<<"\nPrim Time:    "<<pTime;
    out<<kTime<<"\t"<<pTime<<endl;
  }
  //output weights
  //cout<<"\nKruskal Weight: "<<totalWeight(krus,0);
  //cout<<"\nPrim Weight:    "<<totalWeight(prim,0);
