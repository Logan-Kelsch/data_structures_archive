...
template <class T> int AVLTree<T>::height(TreeNode * nodePtr){
    if(!nodePtr)
        return 0;
    return max(height(nodePtr->left),height(nodePtr->right))+1;
}

template <class T> int AVLTree<T>::getHeight(){
    return height(root);
}
...
template <class T> int AVLTree<T>::leafCount(TreeNode *tmp){
    if(tmp){
...
template <class T> double AVLTree<T>::avgIPL(){
    return static_cast<double>(totalIPLRec(root,0))/leafCount(root);
}

template <class T> int AVLTree<T>::totalIPLRec(TreeNode *nodePtr, int height){
    if (nodePtr) {
        height++;
